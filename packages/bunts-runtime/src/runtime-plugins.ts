...
export interface RuntimePlugin {
  ...
  onModuleLoad?: (ctx: PluginContext, id: string, parent?: string) => MaybePromise<ModuleLoadResult | void>
}
...
export class PluginManager {
  ...
  async runModuleLoad(id: string, parent?: string): Promise<ModuleLoadResult | null> {
    let currentId = id
    for (const plugin of this.plugins) {
      if (!plugin.onModuleLoad) continue
      const result = await this.safeCall(() => plugin.onModuleLoad!(this.context(), currentId, parent), plugin, 'onModuleLoad')
      if (!result) continue
      if (result.id) currentId = result.id
      if (result.exports) {
