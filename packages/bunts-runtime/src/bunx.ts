...
export type BunxRunnerOptions = {
  ...
  runtime: BunRuntime
  install: (options: BunInstallOptions) => Promise<void>
  registryUrl?: string
}

export type BunxExecResult = {
  exitCode: number
  entry?: string
  name?: string
  rest?: string[]
...
type BunxPackageSpec = { name: string; version?: string }
...
export function createBunxRunner(options: BunxRunnerOptions) {
  const cache = new Map<string, BunxCacheEntry>()
  return {
    async exec(args: string[]): Promise<BunxExecResult> {
      const parsed = parseBunxArgs(args)
      if (!parsed.name) {
        options.runtime.process.stderr.write('bunx requires a package or binary name.\n')
        return { exitCode: 1 }
      }
      ...
      if (!resolved && shouldAutoInstall(parsed, options.runtime.process.env)) {
        ...
        resolved = resolveBunxBinary(options.vfs, cwd, parsed.name, packageSpec.name, packageSpecs)
        if (resolved) {
      ...
      if (!resolved) {
        options.runtime.process.stderr.write(
          `bunx could not find ${parsed.name} in node_modules or package.json bin.\n`,
        )
        return { exitCode: 1 }
      }
...
function parseBunxArgs(args: string[]): {
  name: string
  rest: string[]
  noInstall: boolean
  packages: BunxPackageSpec[]
  cwd?: string
} {
  let noInstall = false
  let name = ''
  const rest: string[] = []
  const packages: BunxPackageSpec[] = []
...
function resolveBunxBinary(
  vfs: VirtualFileSystem,
  cwd: string,
  name: string,
  packageName: string,
  packageSpecs: BunxPackageSpec[],
): BunxCacheEntry | null {
...
function parsePackageSpecifier(spec: string): BunxPackageSpec {
  if (spec.startsWith('@')) {
...
function isCacheValid(vfs: VirtualFileSystem, entry: BunxCacheEntry): boolean {
