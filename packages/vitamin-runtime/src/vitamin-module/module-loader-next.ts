export * from './module-loader-slim'
/*
...
export class ModuleLoader {
  ...
  async load(entry: string, parent?: string): Promise<ModuleRecord> {
    try {
      ...
      entryRecord.promise = this.loadEntry(entryRecord, resolved, parent)
    ...
    try {
      ...
      if (existing) {
        ...
        if (existing.state === 'loading' || existing.state === 'resolving') {
          throw new Error(`Cannot synchronously load unresolved module: ${resolved}`)
        }
      }
  ...
  private async loadEntry(moduleEntry: ModuleEntry, resolved: string, parent?: string): Promise<ModuleRecord> {
    ...
    try {
      if (this.canUseBrowserImport()) {
        const url = this.resolveImportUrl(resolved)
        const ns = (await import(/* @vite-ignore * / url)) as Record<string, unknown>
        moduleEntry.record.exports = ns
      } else {
  ...
  private resolveImportUrl(resolved: string): string {
