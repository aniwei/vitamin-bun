...
import type { WasiImports, WasmHostOptions } from './types.js'
import {
  WasiErrno,
  WasiFiletype,
  WasiClockId,
...
/**
 * WASI Preview 1 shim implementation.
 *
 * Bridges WASI syscalls to:
 * - VFS for file operations
 * - Browser APIs for clock, random, environment
 * - stdout/stderr callbacks for output
 */
export class WasiShim implements WasiImports {
  ...
  /** Preopened directory fd → path mapping */
  private preopenFds = new Map<number, string>()
  /** fd → VFS path for stat lookups on open fds */
  ...
  /** Must be called after WASM instantiation to bind memory */
  ...
  clock_res_get(id: number, out: number): number {
    this.mem.refresh()
    // 1ms resolution for realtime, 5μs for monotonic (typical browser)
    const resNs =
      id === WasiClockId.REALTIME ? 1_000_000n : 5_000n
  ...
  fd_read(fd: number, iovs: number, iovsLen: number, nread: number): number {
    ...
    if (fd === 0) {
      // stdin — not supported in browser, return 0 bytes
  ...
  fd_fdstat_get(fd: number, buf: number): number {
    ...
    // Minimal implementation: report file type
    if (fd <= 2) {
    ...
    // For VFS files, check if it's a directory or regular file
    try {
  ...
  path_open(
    ...
    try {
      ...
      // Map WASI flags to VFS OpenFlags
      const openFlags = {
        read: true,
  ...
  /** Return a WASI-compatible import object for WebAssembly.instantiate */
...
/** Error thrown by proc_exit to unwind the WASM stack */
