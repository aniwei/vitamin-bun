...
function createShim(overrides: {
  ...
  const shim = new WasiShim({
    ...
    args: overrides.args ?? ['bun', 'run', 'index.ts'],
    preopens: { '/': '/' },
...
describe('WasiShim', () => {
  ...
  describe('args_sizes_get', () => {
    it('returns correct argc and buffer size', () => {
      ...
      const view = new DataView(memory.buffer)
      expect(view.getUint32(argcPtr, true)).toBe(3) // bun, run, index.ts

      // "bun\0" + "run\0" + "index.ts\0"
      const expectedSize =
        new TextEncoder().encode('bun\0').byteLength +
